...
import { updatePredictionStatus } from "@/services/admin-data";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!hasRole(user.role, "ANALYST")) {
    return NextResponse.json(fail("FORBIDDEN", "当前账号无权修改预测状态"), { status: 403 });
  }
  ...
  if (!parsed.success) {
    return NextResponse.json(fail("VALIDATION_ERROR", "预测状态参数错误", parsed.error.flatten()), { status: 422 });
  }
